...
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  casadi_real *w0=w+3, w1, w2, w3, *w4=w+9, *w5=w+12, *w6=w+15, *w7=w+24, w8, w9, *w10=w+29, *w11=w+32, *w12=w+35, *w13=w+38;
  /* #0: @0 = 
  [[0.000251256, 00, 00], 
   [00, 0.000251256, 00], 
   [00, 00, 5.07537e-05]] */
  ...
  *rr++ = w1;
  /* #34: @12 = @12' */
  /* #35: @6 = horzcat(@10, @11, @12) */
  rr=w6;
  for (i=0, cs=w10; i<3; ++i) *rr++ = *cs++;
  for (i=0, cs=w11; i<3; ++i) *rr++ = *cs++;
  for (i=0, cs=w12; i<3; ++i) *rr++ = *cs++;
  /* #36: @13 = @6' */
  for (i=0, rr=w13, cs=w6; i<3; ++i) for (j=0; j<3; ++j) rr[i+j*3] = *cs++;
  /* #37: @5 = mac(@13,@7,@5) */
  for (i=0, rr=w5; i<1; ++i) for (j=0; j<3; ++j, ++rr) for (k=0, ss=w13+j, tt=w7+i*3; k<3; ++k) *rr += ss[k*3]**tt++;
  /* #38: @4 = (@4-@5) */
  for (i=0, rr=w4, cs=w5; i<3; ++i) (*rr++) -= (*cs++);
  /* #39: @0 = (@0*@4) */
  ...
  *rr++ = w1;
  ...
  /* #58: @0 = mac(@6,@7,@0) */
  for (i=0, rr=w0; i<1; ++i) for (j=0; j<3; ++j, ++rr) for (k=0, ss=w6+j, tt=w7+i*3; k<3; ++k) *rr += ss[k*3]**tt++;
  /* #59: output[0][1] = @0 */
