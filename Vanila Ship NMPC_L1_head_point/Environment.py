...
T = 150.00  # maximum simulation time[s]
dt = 0.001
control_time = 1
...
param_estim = np.array([0, 0])
filtered_param = np.array([0,0])
l1_u = np.array([0, 0])
...
for i in range(int(T/dt)):
    ...
    con_input = u0 
    state, V_ship = update_state(state, con_input, dt, np.array([0.0,0.0,0]), np.array([1.0,0.0,0]),i*dt)
    ...
    state_matrix = estim_update_state(x_estim, u_mpc, dt, np.array([px, py]), i*dt)
    x_estim =  state_matrix + np.dot(Am, x_error) 
    ...
    px = Gamma_x*dt*param_dynamics(x_error, param_estim[0], np.array([0, 0, 1, 0, 0, 0])) + px
    py = Gamma_y*dt*param_dynamics(x_error, param_estim[1], np.array([0, 0, 0, math.sin(state[5]), 0, 0])) + py
