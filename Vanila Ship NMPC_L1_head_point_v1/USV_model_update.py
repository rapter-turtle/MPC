...
def estim_update_state(x_t, x_error,  u, dt, param_estim,t):
    ...
    et4dot = uvr_dot[0]*math.sin(psi) + uvr_dot[1]*math.cos(psi) + et3*r + r + uvr_dot[2]*l*math.cos(psi) + Tau_x*math.sin(psi)/m + (1/m + 4*l/Iz)*Tau_y*math.cos(psi) + param_estim[1]
    ...
    Am = -np.eye(6)
    xdot = np.concatenate(([et1dot],[et2dot],[et3dot], [et4dot],[r],[r_uvr_dot[2]]), axis=0) + np.dot(Am, x_error)
