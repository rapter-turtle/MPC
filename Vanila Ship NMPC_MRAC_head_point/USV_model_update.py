...
def estim_update_state(x_t, x_error,  u, l1_u, dt, param_estim,t):
    ...
    l1_y = (l1_u[1]*math.cos(psi) - l1_u[0]*math.sin(psi))/(1/m - 4*l/Iz)
    ...
    et3dot = uvr_dot[0]*math.cos(psi) - uvr_dot[1]*math.sin(psi) - uvr_dot[2]*l*math.sin(psi) - uu*r*math.sin(psi) - v*r*math.cos(psi) - r*r*l*math.cos(psi) + Tau_x*math.cos(psi)/m - (1/m - 4*l/Iz)*Tau_y*math.sin(psi) 

    et4dot = uvr_dot[0]*math.sin(psi) + uvr_dot[1]*math.cos(psi) + uvr_dot[2]*l*math.cos(psi) + uu*r*math.cos(psi) - v*r*math.sin(psi) - r*r*l*math.sin(psi) + Tau_x*math.sin(psi)/m + (1/m - 4*l/Iz)*Tau_y*math.cos(psi) 
...
# et3dot = uvr_dot[0]*math.cos(psi) - uvr_dot[1]*math.sin(psi) - et4*r - uvr_dot[2]*l*math.sin(psi) + Tau_x*math.cos(psi)/m - (1/m + 4*l/Iz)*Tau_y*math.sin(psi) + param_estim[0]