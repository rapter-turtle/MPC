...
/* simple_model_expl_ode_fun:(i0[9],i1[2],i2[])->(o0[9]) */
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  *rr++ = w15;
  ...
  /* #55: @12 = @12' */
  /* #56: @14 = horzcat(@5, @11, @12) */
  rr=w14;
  for (i=0, cs=w5; i<3; ++i) *rr++ = *cs++;
  for (i=0, cs=w11; i<3; ++i) *rr++ = *cs++;
  for (i=0, cs=w12; i<3; ++i) *rr++ = *cs++;
  /* #57: @6 = @14' */
  for (i=0, rr=w6, cs=w14; i<3; ++i) for (j=0; j<3; ++j) rr[i+j*3] = *cs++;
  /* #58: @4 = mac(@6,@7,@4) */
  for (i=0, rr=w4; i<1; ++i) for (j=0; j<3; ++j, ++rr) for (k=0, ss=w6+j, tt=w7+i*3; k<3; ++k) *rr += ss[k*3]**tt++;
  /* #59: output[0][1] = @4 */
  ...
  /* #68: @16 = 0 */
  w16 = 0.;
  /* #69: @4 = horzcat(@10, @15, @16) */
  rr=w4;
  *rr++ = w10;
  *rr++ = w15;
  *rr++ = w16;
  /* #70: @9 = mac(@4,@0,@9) */
  for (i=0, rr=(&w9); i<1; ++i) for (j=0; j<1; ++j, ++rr) for (k=0, ss=w4+j, tt=w0+i*3; k<3; ++k) *rr += ss[k*1]**tt++;
  /* #71: @1 = (@1*@3) */
  w1 *= w3;
  /* #72: @10 = sin(@8) */
  w10 = sin( w8 );
