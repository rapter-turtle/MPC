...
x_estim_before = np.array([l - 10 ,0 ,0 ,0 ,0 ,0])
...
for i in range(int(T/dt)):
    ...
    # u0 = np.array([0,0])
    psi = state[5]
    # l1_u = np.array([-(0.0*math.sin(psi) + 0.03*math.cos(psi))*m,-(0.0*math.cos(psi) - 0.03*math.sin(psi))/(1/m + 4*l/Iz)])
    ...
    x_estim =  estim_update_state(x_estim_before, x_error, u_mpc, dt, np.array([px, py]), i*dt) #+ np.dot(Am, x_error)  
    ...
    px = Gamma_x*dt*param_dynamics(x_error, before_px, np.array([0, 0, 1, 0, 0, 0]), 1000) + before_px
    py = Gamma_y*dt*param_dynamics(x_error, before_py, np.array([0, 0, 0, 1, 0, 0]), 1000) + before_py
    ...
    real[i,:] = x_l1
    GT_dis[i,:] = -np.array([(0.002*math.sin(psi) + 0.003*math.cos(psi))*m, (0.002*math.cos(psi) - 0.003*math.sin(psi))/(1/m + 4*l/Iz)])
...
plotRes(simX, simU, t, sim_l1_con, sim_param, sim_x_estim_error, real, GT_dis)
plotTrackProj(simX)
